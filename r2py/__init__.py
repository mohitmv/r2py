...
def create_graph():
	...
	# 	for j in inp:
	# 		elm = nodes[j]['tokens'][0];
	# 		if(is_x_inside_y(elm, i)):
	# 			edges[elm['node_id']].add(i['node_id']);
	# 		if(is_x_inside_y(i, elm)):
	# 			edges[i['node_id']].add(elm['node_id']);
	...
	# for i in nodes:
	# 	nodes[i]['out_going'] = edges[i];

	# for i in edges:
	# 	for j in edges[i]:
	# 		nodes[j]['in_coming'].add(i);
